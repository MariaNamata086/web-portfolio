...
export default function Playground() {
  ...
  return (
    <section id='playground' className='px-5.5 py-16 md:px-10 md:py-25'>
      <div className='mx-auto max-w-site'>
        ...
        <Reveal variant='rvZ'>
          <div className='grid items-start gap-9 rounded-panel bg-forest p-6 text-on-forest md:grid-cols-2 md:p-9'>
            <div>
              <h3 className='font-display text-[26px] font-bold tracking-[-0.03em]'>Live design tokens</h3>
              ...
              <div className='mt-5.5'>
                <span className='mb-2.5 block font-mono text-[10px] tracking-widest uppercase opacity-70'>Accent</span>
                <div className='flex flex-wrap gap-2.5'>
                  {presets.map((p, i) => (
                    <button
                      ...
                      className={`h-10 w-10 cursor-pointer rounded-xl border-2 transition-transform hover:scale-110 ${picked === i ? 'border-on-forest' : 'border-transparent'}`}
              ...
              <div className='mt-5.5'>
                <label htmlFor='radius' className='mb-2.5 block font-mono text-[10px] tracking-widest uppercase opacity-70'>
                  Corner radius, {radius}px
                </label>
                <input id='radius' type='range' min={0} max={34} step={1} value={radius} onChange={(e) => setR(Number(e.target.value))} className='w-full accent-ochre' />
              </div>

              <div className='mt-5.5'>
                <span className='mb-2.5 block font-mono text-[10px] tracking-widest uppercase opacity-70'>Motion</span>
            ...
            <div>
              <span className='mb-2.5 block font-mono text-[10px] tracking-widest uppercase opacity-70'>Generated tokens</span>
              <div className='overflow-x-auto rounded-2xl border border-white/15 bg-black/20 p-4.5 font-mono text-[12px] leading-[1.9]'>
                {tokens.map(([k, v]) => (
                  <div key={k}>
                    {k}: <span className='text-ochre'>{v}</span>
                  </div>
                ))}
              </div>
              <button
                ...
                className='mt-4 rounded-pill bg-ochre px-6 py-3 text-[15px] font-semibold text-on-ochre'
              >
